...
public class PlayerPhysics : MonoBehaviour {
	...
	public void Move(Vector2 moveAmount) {
		...
		#region Top and Bottom Collisions
		...
		for (int i = 0; i<collisionDivisionX; i ++) {
			...
			if (Physics.Raycast(ray,out hit,Mathf.Abs(deltaY) + clearance,collisionMask)) {
		...
		for (int i = 0; i<collisionDivisionY; i ++) {
			...
			Debug.DrawRay(ray.origin,ray.direction);
			if (Physics.Raycast(ray,out hit,Mathf.Abs(deltaX) + clearance,collisionMask)) {
				// Get Distance between player and ground
				float dst = Vector3.Distance (ray.origin, hit.point);
				
				// Stop player's downwards movement after coming within skin width of a collider
				if (dst > clearance) {
					deltaX = dst * dir - clearance * dir;
				}
				else {
					deltaX = 0;
				}
